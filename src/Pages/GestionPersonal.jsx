...
const STAFF_TYPES = ['Veterinario', 'Asistente', 'Logística', 'Captador', 'Otro']

// Pills de categoría (Cargo) — el value vacío ('Todos') no envía filtro
// staff_type al backend. El modelo Staff no tiene un campo de estado
// activo/inactivo, así que el cargo es la categoría relevante disponible.
const CATEGORIAS_CARGO = [
  { label: 'Todos', value: '' },
  ...STAFF_TYPES.map(tipo => ({ label: tipo, value: tipo })),
]

const FORMULARIO_VACIO = { first_name: '', last_name: '', dni_number: '', phone: '', staff_type: '' }
...
export default function GestionPersonal({ readOnly = false }) {
  ...
  // Se recarga cada vez que cambia la página, la búsqueda aplicada o el cargo
  useEffect(() => {
    cargarPersonal()
  ...
  const cargarPersonal = async () => {
    try {
      ...
      const respuesta = await staffService.getAll({
        page: pagina,
        limit: REGISTROS_POR_PAGINA,
        search: busquedaAplicada,
        staff_type: categoriaActiva,
      })

      const cuerpo = respuesta.data || {}
      setPersonal(cuerpo.data || [])
      setTotalPaginas(cuerpo.metadata?.totalPages || 1)
      setTotalRegistros(cuerpo.metadata?.totalRecords || 0)
    } catch (err) {
      setError('Error al cargar el personal: ' + (err.response?.data?.message || err.message))
  ...
  const guardar = async (e) => {
    ...
    if (!formulario.first_name.trim() || !formulario.last_name.trim() || !formulario.dni_number.trim() || !formulario.staff_type) {
      setErrorForm('Nombre, apellido, cédula y tipo de personal son obligatorios.')
      return
    }
    ...
    try {
      ...
      if (editando) {
        await staffService.update(editando, formulario)
      } else {
        await staffService.create(formulario)
      }
  ...
  const eliminar = async (p) => {
    if (!window.confirm(`¿Está seguro de que desea eliminar a ${p.first_name} ${p.last_name}?`)) return

    try {
      setCargando(true)
      await staffService.delete(p.id_staff)
      await cargarPersonal()
    } catch (err) {
  ...
  // Cambia el cargo activo y vuelve a la página 1 — un filtro nuevo invalida
    ...
    return (
      <div className="space-y-5 max-w-3xl">
        ...
        >
          ...
          Volver al Registro de Personal
        </button>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-bold text-gray-800 tracking-tight">
              {editando ? 'Editar Personal' : 'Registrar Nuevo Personal'}
            </h2>
            <p className="text-sm text-gray-400 mt-0.5">Complete los datos del personal para vincularlo en el sistema.</p>
          </div>
        </div>
          ...
          <form onSubmit={guardar} className="p-7 space-y-6">
            <div>
              ...
              <div className="grid grid-cols-2 gap-5">
                ...
                <Campo label="Apellido (last_name)">
                  <input
                    type="text"
                ...
                <Campo label="Cédula de Identidad (dni_number)">
                  <input
                    type="text"
            ...
            <div>
              <p className="text-xs font-bold text-[#765A05]/70 uppercase tracking-[0.18em] mb-4">Rol en el Sistema</p>
              <div>
                <Campo label="Tipo de Personal (staff_type)">
                  <div className="relative">
                    <Briefcase className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400 pointer-events-none" />
                    <select
                      value={formulario.staff_type}
                      onChange={e => cambio('staff_type', e.target.value)}
    ...
    <div className="space-y-6 max-w-6xl">
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-bold text-gray-800 tracking-tight">Registro de Personal</h2>
            <p className="text-sm text-gray-400 mt-0.5">Personal registrado en el sistema SISCVI — Misión Nevado</p>
          </div>
        </div>
        {!readOnly && (
          <button
      ...
      <div className="bg-white/60 backdrop-blur-md border border-white/60 shadow-xl shadow-[#765A05]/5 rounded-3xl overflow-hidden">
        ...
        <div className="px-5 py-4 border-b border-gray-100/70">
          <FiltrosPills opciones={CATEGORIAS_CARGO} activo={categoriaActiva} onCambio={seleccionarCategoria} />
        </div>
          ...
          <table className="w-full min-w-[800px]">
            <thead>
              <tr className="bg-[#FFDF96]/10 border-b border-gray-100/70">
                {['Nombre Completo', 'Teléfono', 'Tipo de Personal', 'Cédula', 'Acciones'].map((col, i) => (
                  <th
                    key={col}
              ...
              {cargando ? (
                <tr>
                  <td colSpan={5} className="py-20 text-center">
                    ...
                    <p className="text-sm text-gray-400 font-medium">Cargando personal...</p>
                  </td>
                </tr>
              ) : personal.length === 0 ? (
                <tr>
                ...
                personal.map(p => (
                  <tr key={p.id_staff} className="border-b border-gray-50/80 last:border-0 hover:bg-[#FFDF96]/8 transition-colors">
                    ...
                    <td className="px-5 py-3.5 whitespace-nowrap">
                      <div className="flex items-center gap-1.5">
                        ...
                        <p className="text-sm text-gray-600 font-mono tracking-wide">{p.phone || '—'}</p>
                      </div>
                    </td>
                    <td className="px-5 py-3.5 whitespace-nowrap">
                    ...
                    <td className="px-5 py-3.5 whitespace-nowrap">
                      <p className="text-sm text-gray-600">{p.dni_number || '—'}</p>
                    </td>
                    <td className="px-5 py-3.5">
